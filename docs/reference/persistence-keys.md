...
| `instatic-editor-layout-v2`                     | Per-workspace sidebar widths + open states (site / content / data / media) and floating panel positions, plus Code Dock sizes, visibility and panel order | `src/admin/state/workspaceLayoutStorage.ts` → `EDITOR_LAYOUT_STORAGE_KEY` |
...
| `instatic-css-toolbar-order`                  | CSS toolbar control order, shared by panels and retained across browser sessions | `src/admin/pages/site/code-dock/css/cssToolbarOrder.ts` |
